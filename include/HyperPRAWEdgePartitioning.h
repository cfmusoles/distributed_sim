...
public:
	...
	HyperPRAWEdgePartitioning(std::vector<Population*>* pops, int population_size, char* comm_bandwidth_file, bool useBandwidth) : Partitioning(pops,population_size) {
	...
	virtual void perform_partitioning(Model* model, int partitions, int process_id, std::vector<int>* previous_activity) {
        ...
        PRAW::ParallelHDRF(experiment_name,partitioning, comm_cost_matrix, hgraph_file.c_str(), he_wgt, max_iterations, imbalance_tolerance, true);
