...
def plot_isi_hist(panel, segment, label, hide_axis_labels=False,ymax=None):
...
def plot_cvisi_hist(panel, segment, label, hide_axis_labels=False,ymax=None):
...
def plot(datafiles, output_file, sort_by='simulator', annotation=None):
    ...
    for k, (label, block_list) in enumerate(sorted_blocks.items()):
        ...
        plot_isi_hist(plt.subplot(gs[0, 2 * k]), segments['exc'], 'exc', hide_axis_labels,1800)
        plot_cvisi_hist(plt.subplot(gs[1, 2 * k]), segments['exc'], 'exc', hide_axis_labels,500)
        #hide_axis_labels = True
        plot_isi_hist(plt.subplot(gs[0, 2 * k + 1]), segments['inh'], 'inh', hide_axis_labels,500)
        plot_cvisi_hist(plt.subplot(gs[1, 2 * k + 1]), segments['inh'], 'inh', hide_axis_labels,120)
